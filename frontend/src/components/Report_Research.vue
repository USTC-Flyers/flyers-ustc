  ...
  <div>
    ...
    <div class="form">
      ...
      <div v-show="active == 1">
        ...
        >
          <!-- <el-button
                v-if="index > 0 || researchs.length > 1"
                type="danger"
                size="mini"
                @click="del_research(index)"
                >删除此项</el-button
              > -->
          <el-button
            v-if="index > 0 || researchs.length > 1"
        ...
        <!-- <el-form
              ...
              v-for="(item, index) in researchs"
              :key="index"
              :model="researchs[index]"
              :id="index"
              ref="research_forms"
              label-position="right"
              label-width="90px"
            >
              <el-form-item v-show="index > 0 || researchs.length > 1" label=" ">
                <el-button type="danger" size="mini" @click="del_research(index)"
                  >删除此项</el-button
                >
              </el-form-item>
              <el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="录取学校" prop="related_university">
                    <el-select
                      ...
                      @change="researchs[index].related_program = null"
                      ><el-option
                        class="university"
              ...
              <el-form-item v-show="index == researchs.length - 1" label=" ">
                <el-button type="success" size="small" @click="add_research()"
                  >新增录取项</el-button
                >
              </el-form-item>
            </el-form> -->
...
export default {
  ...
  data() {
    return {
      ...
      research_rules: {
        ...
        enrolledSemester: [
          { required: true, message: "请选择暑研时间", trigger: "blur" },
        ],
        related_university: [
  ...
  computed: {
    final_univ_list() {
      ...
      for (let i = 0; i < this.univ_list.length; i++) {
        if (this.univ_list[i][0] && this.researchs[i].result) {
          result.push({
            univ: this.univ_list[i][0],
            program: this.researchs[i].related_program,
          });
        }
      }
  ...
  methods: {
    add_research() {
      this.researchs.push({
        result: null,
        enrolledSemester: this.researchs[0].enrolledSemester,
        related_university: null,
        related_program: null,
        comments: "",
        summary: "",
    ...
    del_research(index) {
      if (this.researchs[index].id) {
        this.delete_ids.push(this.researchs[index].id);
      }
    ...
    //   var uid = this.researchs[index].related_university;
    ...
    getData() {
      if (this.is_initial == false) {
        ...
        research_get_my().then((response) => {
          let data = response.user_detail;
          for (let i = 0; i < data.length; i++) {
            this.$set(this.researchs, i, {
    ...
    fillFinalProgram(index) {
      console.log(index);
      this.final_program = this.researchs[index].related_program;
    },
    async handleSubmit() {
      this.form_data.background.tags =
        this.form_data.background.research_tag_list.concat(
          this.form_data.background.ref_tag_list
        );
      if (!this.form_data.background.major) {
        this.form_data.background.major = null;
      }
      var re_forms = this.$refs["research_forms"];
      var valid_funcs = [];
      re_forms.forEach((form, index) => {
        valid_funcs.push(
