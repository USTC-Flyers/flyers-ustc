<template>
    <div class="codediff">
        <h>Wiki标题</h>
        <code-diff :old-string="old_title" :new-string="new_title" :context="10" outputFormat="side-by-side"/>
        <h>Wiki内容</h>
        <code-diff :old-string="old_content" :new-string="new_content" :context="10" outputFormat="side-by-side"/>
...
import {getTopicRevision, getTopic, approveTopic, rejectTopic} from "@/api/wiki"
...
export default {
    ...
    created() {
        ...
        getTopicRevision(this.revision_id).then((resp) => {
    ...
    methods: {
        revisionAp() {
            approveTopic(this.topic_id).then(() => {
        ...
        revisionRej() {
            rejectTopic(this.topic_id).then(() => {
